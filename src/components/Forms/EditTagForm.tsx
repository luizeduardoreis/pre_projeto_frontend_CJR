...
export default function EditTagForm({ tag }: EditTagForm) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const updateTag = async (data: EditTagFormData) => {
    tag.name = data.name;

    const res = await fetch(`http://192.168.1.100:8181/tags/${tag.id}`, {
      method: "PATCH",
      headers: {
  ...
  return (
    <form
      onSubmit={handleSubmit(updateTag)}
