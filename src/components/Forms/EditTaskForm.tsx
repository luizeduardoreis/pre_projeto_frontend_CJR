...
export default function EditTaskForm({ task, tags }: EditTaskFormProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const editTask = async (data: EditTaskFormData) => {
    task.name = data.name;
    task.tagId = data.tagId;
  ...
  return (
    ...
    >
      <div className="flex flex-col">
        ...
        <select
          id="small"
          className="w-full p-2 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-5"
          {...register("tagId")}
        >
          <option defaultValue={undefined} value={undefined}>Escolha uma opção</option>
          {tags.map((tag) => {
            return <TagSelectItem tag={tag}></TagSelectItem>;
          })}
        </select>
      </div>
      <div className="flex items-center justify-center gap-10">
        ...
        <div>
          <button
            type="submit"
            className="text-white bg-gradient-to-r from-pink-400 via-pink-500 to-pink-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-pink-300 dark:focus:ring-pink-800 font-medium rounded-lg text-sm px-12 py-2 text-center"
          >
            Salvar Edição
          </button>
        </div>
